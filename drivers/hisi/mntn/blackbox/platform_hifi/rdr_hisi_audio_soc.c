...
static int rdr_hifi_cma_alloc(phys_addr_t *hifi_addr)
{
	...
	page = dma_alloc_from_contiguous(dev->device, HIFI_CMA_IMAGE_SIZE >> PAGE_SHIFT, 0);
	if (!page) {
	...
	*hifi_addr = phys;
	BB_PRINT_PN("rdr:%s(): address 0x%x\n", __func__, (unsigned int)phys);
	return 0;
}
...
static int reset_hifi(void)
{
	...
	if (ret != 0) {
		...
		(void)rdr_hifi_cma_free(hifi_addr);
		goto err;
	}
	...
	ret = rdr_hifi_cma_free(hifi_addr);
	if (ret != 0)
		goto err;
