...
static HI_S32 processor_report_frame(OMXVDEC_CHAN_CTX *pchan, BPP_VIDEO_FRAME_S *pstFrame)
{
    ...
    memset(&user_buf, 0, sizeof(OMXVDEC_BUF_DESC));
    memset(&pbuf->ext_info, 0, sizeof(EXT_INFO));
    ...
    {
        memcpy(user_buf.out_frame.act_crc, pstFrame->image.ActCRC, sizeof(user_buf.out_frame.act_crc));
    }
...
static HI_S32 processor_get_image(OMXVDEC_CHAN_CTX *pchan, BPP_VIDEO_FRAME_S *pstFrame)
{
    ...
    if (1 == pchan->spec_mode)
    {
        memcpy(pstFrame->image.ActCRC, stImage.ActualCRC, sizeof(pstFrame->image.ActCRC));
    }
...
static HI_S32 processor_restore_frame_without_gap(BPP_VIDEO_FRAME_S *pSrcFrame, BPP_VIDEO_FRAME_S *pDstFrame)
{
    ...
    memcpy(&pDstFrame->image, &pSrcFrame->image, sizeof(BPP_IMAGE_INFO_S));
    
    /*restore y*/
    for (i=0; i<Height; i++)
    {
        memcpy(pDstAddr, pSrcYAddr, Width);
        pSrcYAddr += SrcStride;
        pDstAddr  += DstStride;
    }
    ...
    {
        memcpy(pDstAddr, pSrcCAddr, Width);
        pSrcCAddr += SrcStride;
        pDstAddr  += DstStride;
    }
...
static HI_S32 processor_work_in_normal_mode(OMXVDEC_CHAN_CTX *pchan, BPP_CONTEXT_S *pBppContext)
{
    ...
    memset(&stOutFrame, 0x0, sizeof(BPP_VIDEO_FRAME_S));
...
static HI_S32 processor_work_in_bypass_mode(OMXVDEC_CHAN_CTX *pchan, BPP_CONTEXT_S *pBppContext)
{
    ...
    // get image
    memset(&stFrame, 0, sizeof(BPP_VIDEO_FRAME_S));
...
{
    VDEC_DOWN_INTERRUPTIBLE(&gBPPSem);

    memset(gpBPPContext, 0, MAX_CHANNEL_NUM * sizeof(BPP_CONTEXT_S*)); /* [false alarm] */
    g_BPPInit = 1;

    VDEC_UP_INTERRUPTIBLE(&gBPPSem);
...
HI_S32 processor_create_inst(OMXVDEC_CHAN_CTX *pchan, HI_U32 color_format)
{
    ...
    else
    {
        ...
        snprintf(name_array, sizeof(name_array), "Chan%d_BPP", pchan->channel_id);
        ...
        memset(mem_buf.pStartVirAddr, 0, mem_buf.u32Size);
        
        pBppContext = (BPP_CONTEXT_S *)mem_buf.pStartVirAddr;
        memcpy(&pBppContext->mem_buf, &mem_buf, sizeof(MEM_BUFFER_S));
        ...
        snprintf(name_array, sizeof(name_array), "BPP_Thread_%d", i);
...
HI_S32 processor_reset_inst(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    memset(&pBppContext->stFrame, 0, sizeof(BPP_VIDEO_FRAME_S));    
